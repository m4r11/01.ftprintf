...
char field_x(char *dir, va_list args2, int flag)
{
    ...
    if (dv.temp[0] == '-' && ft_isdigit(dv.temp[1]) && dv.temp[2] == '.')
    {
        /*   TESTE 4 5*/
        //debug_number(dv.converted, "c");
        dv.min_c = ft_atoi(&dv.temp[3]);
        //debug_number(dv.min_c, "m");
        //debug_number(ft_xlen(dv.hex), "l");

        if (ft_xlen(dv.hex) > dv.min_c)
        {
            print_x_times(ft_xlen(dv.hex) - dv.min_c, '0');
            ft_put_x(dv.hex);
            print_x_times(dv.converted - dv.min_c, ' ');
            return (0);
        }
        print_x_times(dv.min_c - ft_xlen(dv.hex), '0');
        ft_put_x(dv.hex);
        print_x_times(dv.converted - dv.min_c, ' ');
    ...
    if (ft_isdigit(dv.temp[0]) && dv.temp[2] == '.')
    {
        /*   TESTE 6 */
        dv.min_c = ft_atoi(&dv.temp[3]);
        if (dv.x == 32767)
